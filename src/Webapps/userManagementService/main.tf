...
locals {
  ...
  app_service_name  = "wcp-${local.environmentPrefix}-${var.location_environmentPrefix}-usermanagementservice-app"
}
...
resource "azurerm_app_service" "app_service_name" {
  ...
  site_config {
    #linux_fx_version  = "DOCKER|wcpgovwus2acr.azurecr.io/usermanagement-service:latest"
    always_on              = "true"
    scm_type               = "VSTSRM"
  ...
  app_settings = {
    ...
    RoleName                                        = "UserManagementService"
  }
}
