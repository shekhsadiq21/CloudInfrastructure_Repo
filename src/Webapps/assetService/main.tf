locals {
  ...
  app_service_name  = "wcp-${local.environmentPrefix}-${var.location_environmentPrefix}-assetservice-app"
}
...
resource "azurerm_app_service" "app_service_name" {
  ...
  site_config {
    linux_fx_version = "DOCKER|wcpgovwus2acr.azurecr.io/asset-service:latest"
    always_on              = "true"
