locals {
  ...
  function_app_name = "wcp-${local.environmentPrefix}-${var.location_environmentPrefix}-virtualexpressiondeploymentverification-func"
}
...
resource "azurerm_function_app" "function_app" {
  ...
  site_config {
    # linux_fx_version = "DOCKER|wcpgovwus2acr.azurecr.io/virtualexpressiondeploymentverification-service:latest"
    always_on              = "true"
  ...
  app_settings = {
    ...
    EventHubName                        = "eventhub-twinchangeevent"
    ...
    ModuleId                            = "EdgeConfig"    
    RoleName                            = "VirtualExpressionDeploymentVerificationService"
    ModulePropertyName                  = "virtualEventGenerator"
