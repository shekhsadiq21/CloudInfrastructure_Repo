locals {
  ...
  function_app_name = "wcp-${local.environmentPrefix}-${var.location_environmentPrefix}-virtualexpressiontester-func"
}
...
resource "azurerm_function_app" "function_app" {
  ...
  site_config {
    # linux_fx_version = "DOCKER|wcpgovwus2acr.azurecr.io/virtualexpressiontester-service:latest"
    always_on              = "true"
  ...
  app_settings = {
    ...
    ServiceBusSubscriptionName          = "sbs-eventmanagement-virtualevent_testing"
    ServiceBusTopicName                 = "sbt-virtualevent-testing-command"
    FUNCTIONS_WORKER_RUNTIME            = "dotnet"    
    RoleName                            = "VirtualexpressionTester"
    StorageConnectionString             = var.storage_primary_connection_string
    VirtualBuilderContainer             = "virtual-builder"
